...
class MGICNN:
    ...
    def _out_layer(self, inputs):
        ...
        pred_sig = tf.nn.sigmoid(logit)
        prediction = tf.cast(tf.round(pred_sig), tf.int64)[...,0]
    ...
    def build_proposed(self):
        ...
        self.loss = tf.losses.sigmoid_cross_entropy(multi_class_labels=self.placeholders["lbl"],
                                               logits=logit)
        if st.train:
